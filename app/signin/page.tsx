...
const SigninPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else if (!isValidEmail(form.email)) newErrors.email = "Correo electrónico no válido";
    ...
    try{
      const response = await api.post<AuthResponse>("/FineWise/auth/login-user", {
        email: form.email,
        password: form.password,
      });
      localStorage.setItem("token", response.data.accessToken); // Store token in localStorage
      alert("¡Inicio de sesión exitoso!");
      router.push("/dashboard"); // Redirect to dashboard after successful loginq
    }catch (error){
